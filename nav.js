...
function criarHeader() {
    ...
    // Botão de login ou ícone + botão sair
    let areaLogin = tipoUsuario
        ? `<div class="usuario-logado" title="Logado">
    ...
    // Botão carrinho (esconde na própria página do carrinho)
    let btnCarrinho = paginaAtual !== "carrinho.html"
        ? `<a href="carrinho.html" class="botao-carrinho">🛒 Carrinho</a>`
        : "";

    // Painel exclusivo de fornecedor (abaixo do header)
    ...
    // Header — Relatório NÃO entra aqui, fica só no painel
    let headerHTML = `
    <header>
        <div class="logo">
    ...
    // Scroll suave no contato (só funciona na index)
